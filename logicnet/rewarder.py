...
class LogicRewarder:
    def __init__(self, base_url: str, api_key: str, model: str):
        ...
        """
        bt.logging.info(
            f"Logic Rewarder initialized with model: {
    ...
    def __call__(self, uids, responses: list[LogicSynapse], base_synapse: LogicSynapse):
        ...
        if valid_uids:
            ...
            response_texts = [
                response.logic_reasoning for response in valid_responses]
            similarities = self._get_similarity(
                ref_ground_truth, response_texts)
            ...
            for i in range(len(valid_responses)):
                ...
                reward = reward / 2 + 0.5
                bt.logging.debug(
                    f"[REWARDER] similarity: {similarities[i]}, correctness: {
                        correctness[i]}, processing time: {process_times[i]}"
                )
    ...
    def _get_ground_truth(self, question: str):
        ...
        bt.logging.debug(
            f"[SIMILARITY] Self-generated ground truth: {response}")
        return response
